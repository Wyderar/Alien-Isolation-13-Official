...
/obj/structure/closet/proc/xeno_toggle(mob/user as mob)
	if (opened)
		return toggle(user)//the xeno can always close it
	...
	for (var/mob/m in src)//you want to hide a dog from the xenos or something
		if (m.seen_by_hive)//means we can also always open it if another xeno is in there
			return toggle(user)

	src << "<span class='alium'>You don't sense anything in there.</span>"
...
/obj/structure/closet/attack_hand(mob/user as mob)
	if ("species" in user.vars)
		if (user:species.attack_hand_special(src) == TRUE)
			src.add_fingerprint(user)
			return TRUE
