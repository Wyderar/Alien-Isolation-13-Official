...
/obj/structure/window/attackby(obj/item/W as obj, mob/user as mob)
	...
	var/destructable = 	(!is_exterior() || !config.impenetrable_station_exterior)

	if(istype(W, /obj/item/weapon/screwdriver) && destructable)
		if(reinf && state >= 1)
			state = 3 - state
			update_nearby_icons()
	...
	else if(istype(W, /obj/item/weapon/crowbar) && reinf && state <= 1 && destructable)
		state = 1 - state
		playsound(loc, 'sound/items/Crowbar.ogg', 75, 1)
		user << (state ? "<span class='notice'>You have pried the window into the frame.</span>" : "<span class='notice'>You have pried the window out of the frame.</span>")
	else if(istype(W, /obj/item/weapon/wrench) && !anchored && (!state || !reinf) && destructable)
		if(!glasstype)
