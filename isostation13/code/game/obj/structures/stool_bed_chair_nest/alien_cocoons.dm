//Alium cocoons. Essentially beds with an unbuckle delay that only aliums can buckle mobs to.
#define COCOON_RESIST_TIME 1200
...
/obj/structure/bed/cocoon
	name = "alien cocoon"
	desc = "It's a gruesome pile of thick, sticky resin shaped like a cocoon."
	icon = 'icons/mob/alien_new.dmi'
	icon_state = "cocoon_fresh"
	var/health = 100
	var/progress = 0
...
/obj/structure/bed/cocoon/Destroy()
	clear_overlays()
	processing_objects -= src
	..()

/obj/structure/bed/cocoon/proc/stages_need_reset()
	for (var/v = STAGE_FIRST, v <= STAGE_LAST, v++)
		if (stages[v] != FALSE)
			return TRUE
	return FALSE

/obj/structure/bed/cocoon/proc/resin_overlay()
	if (!stages[STAGE_RESIN_OVERLAY_ADDED])
		overlays += /obj/structure/bed/cocoon/overlay
		stages[STAGE_RESIN_OVERLAY_ADDED] = 1

/obj/structure/bed/cocoon/proc/clear_overlays()
	overlays = list()


/obj/structure/bed/cocoon/proc/reset_stages()
...
/obj/structure/bed/cocoon/process()//probably shouldn't use ..()
	if (!buckled_mob)
		progress = 0
		reset_stages()
		if (overlays.len)
			...
			/*
				...
				if ((prob(3) && !stages[STAGE_ABSORBTION]) || (progress > 110 && !stages[STAGE_ABSORBTION]))//probably once or twice? At least once
					...
					buckled_mob.visible_message(\
						...
						"<span class='danger'>[src] fully contracts around your body! You've been absorbed by the cocoon!</span>",\
...
/obj/structure/bed/cocoon/update_icon()
	return

/obj/structure/bed/cocoon/user_unbuckle_mob(mob/user as mob)
	if (stages[STAGE_ABSORBTION]) return
	if(buckled_mob)
		if(buckled_mob.buckled == src)
			if(buckled_mob != user)
				buckled_mob.visible_message(\
					"<span class='notice'>[user.name] pulls [buckled_mob.name] free from the cocoon!</span>",\
					"<span class='notice'>[user.name] pulls you free from the resin cocoon.</span>",\
				...
				buckled_mob.visible_message(\
					"<span class='warning'>[buckled_mob.name] struggles to break free of the resin cocoon...</span>",\
					"<span class='warning'>You struggle to break free from the resin cocoon...</span>",\
					"<span class='notice'>You hear squelching...</span>")
				spawn(COCOON_RESIST_TIME)
...
/obj/structure/bed/cocoon/user_buckle_mob(mob/M as mob, mob/user as mob)
