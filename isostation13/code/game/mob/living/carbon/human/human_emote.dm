/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null, forced = 0)

	if (act == "hiss")
		act = "hisss"//prevent removal of terminating s
...
wink, yawn, swish, sway/wag, fastsway/qwag, stopsway/swag"}
			...
			if (isalien(src))
				...
				switch (alien_emote)
					if ("hiss")
						playsound(src, "x_a_hiss", 100)
						message = "hisses."
						m_type = 2
					if ("roar")
						playsound(src, "x_a_roar", 100)
						message = "roars!"
						m_type = 2
					if ("snarl")
						playsound(src, "x_a_snarl", 100)
						message = "snarls."
						m_type = 2
				goto _message_
