...
/mob/living/carbon/human/IsAdvancedToolUser(var/silent)
	if(species.has_fine_manipulation || !species)
		return TRUE
	if(!silent)
...
/mob/living/carbon/human/proc/CanPickUpStuff(var/silent)
	if(species.can_pick_up_stuff || !species)
		return TRUE
	if(!silent)
		src << "<span class='warning'>You don't have the dexterity to use that!</span>"
	return FALSE

/mob/living/carbon/human/proc/CanPickUp(var/obj/i)
	if (!CanPickUpStuff(TRUE))
		if (ishumanoidalien(src))
...
/mob/living/carbon/human/proc/handle_prydoor(var/obj/machinery/door/d)
	visible_message("<span class='alium'><B>[src] tries to pry open the door!</B></span>")
	if(stat)
		src << "You must be conscious to do this!"
		return
	if(lying)
		src << "You can't pry a door while you're stunned!"
		return


	if(d) // one was passed in, probably from vent/AltClick()
		if(!d.Adjacent(src))
	...
	if (!istype(d, /obj/machinery/door/window))
		...
		for(var/mob/O in viewers(src, null))
			spawn(rand(3,5))
				if (d.Adjacent(src))
					var/initial_loc = get_turf()
					if (!istype(d, /obj/machinery/door/blast))
						spawn (10)
							if (get_turf() != initial_loc) return
							d.forced_open()
							if (d.p_open)
								O.show_message(text("<b>[src] pries open [d]!</b>"), 1)
					else
						spawn (20)//plus 1.5 seconds it takes for actually opening
							if (get_turf() != initial_loc) return
