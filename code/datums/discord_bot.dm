...
/hook/startup/proc/initialize_discord_bot()
	...
	discord_bot.send_to_announce("Hello, world!")

	if (config.debug_mode_on)

		ASSERT(discord_bot.auth_token == "Mjk2MDk2OTI2MDUzNTY0NDE2.C7tXIA.jS3LZI9lgEdK4KuHsbLo_tOz5fY")

	return TRUE

/datum/discord_bot

	var/list/channels_to_group = list(CHAN_ADMIN = null, CHAN_ANNOUNCE = null)		// Group flag -> list of channel datums map.
	var/list/channels = list("ADMIN" = null, "GENERAL" = null)				// Channel ID -> channel datum map. Will ensure that only one datum per channel ID exists.
	...
	var/channel_ids = list("GENERAL" = "274310484004438018", "ADMIN" = "276084934890749952")
	var/server_ids = list("MAIN" = "274310484004438018")
	var/pin_ids = list("NONE" = null)
...
///datum/discord_channel/New(var/_id, var/_sid, var/_pin)
	...
	*/
	if (isnull(channels_to_group[CHAN_ADMIN]))
		channels_to_group[CHAN_ADMIN] = list()

		var/datum/discord_channel/B = channels["ADMIN"]

		if (isnull(B))
			B = new(channel_ids["ADMIN"], server_ids["MAIN"], pin_ids["NONE"])


			channels["ADMIN"] = B


		channels_to_group[CHAN_ADMIN] += B

	if (isnull(channels_to_group[CHAN_ANNOUNCE]))
		channels_to_group[CHAN_ANNOUNCE] = list()

		var/datum/discord_channel/B = channels["GENERAL"]

		if (isnull(B))
			B = new(channel_ids["GENERAL"], server_ids["MAIN"], pin_ids["NONE"])

			channels["GENERAL"] = B


		channels_to_group[CHAN_ANNOUNCE] += B
	...
	if (!isnull(channels_to_group[CHAN_INVITE]))
		...
		log_debug("BOREALIS: No invite channel designated.") */ // END DISABLED
...
/datum/discord_bot/proc/send_message(var/channel_group, var/message)
	...
	for (var/B in A)
		...
		if (channel.send_message_to(auth_token, message) == SEND_TIMEOUT)
			...
			if (config.debug_mode_on)
				world << "BOREALIS: Message sent to [channel_group]. JSON body: '[message]'"
			sent += channel
...
/datum/discord_channel/proc/send_message_to(var/token, var/message)
	...
	if (config.debug_mode_on)
		ASSERT(id == "274310484004438018" || id == "276084934890749952")
