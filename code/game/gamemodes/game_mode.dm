...
/datum/game_mode
	...
	var/equalize_antag_spawntargets = FALSE
...
/datum/game_mode/proc/pre_setup()
	...
	var/max_antags_per_tag = null //use default.

	if (equalize_antag_spawntargets)
		var/tags = 0

		for (var/datum/antagonist/antag in antag_templates)
			++tags

		max_antags_per_tag = max(round(num_players()/tags), 1)
	...
	for(var/datum/antagonist/antag in antag_templates)
		...
		if(antag.flags & ANTAG_OVERRIDE_JOB)
			antag.attempt_spawn(max_antags_per_tag) //select antags to be spawned
